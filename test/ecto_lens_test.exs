defmodule EctoLensTest do
  ...
  describe "table_schema/0" do
    test "returns specified table schema" do
      ...
      assert :ok = Application.put_env(:ecto_lens, :table_schema, "a_custom_schema_prefix")
      assert "a_custom_schema_prefix" = EctoLens.table_schema()
      ...
      assert :ok = Application.delete_env(:ecto_lens, :table_schema)
      assert "public" = EctoLens.table_schema()
    end
  end
  ...
  describe "list_tables/2" do
    test "returns error when given non-ecto repo" do
      assert_raise(
        ...
        fn -> EctoLens.list_tables(Enum) end
      )
    end
    ...
    test "returns error when given ecto repo, but unsupported adapter" do
      assert_raise(
        ...
        Given: :test_adapter
        """,
        fn -> EctoLens.list_tables(Test.BadRepo) end
      )
    end
  end
  ...
  describe "get_table/3" do
    test "returns error when given non-ecto repo" do
      assert_raise(
        ArgumentError,
        "Expected a module that `use`-es `Ecto.Repo`, got: `Enum`",
        fn -> EctoLens.get_table(Enum, "users") end
      )
    end
    ...
    test "returns error when given ecto repo, but unsupported adapter" do
      assert_raise(
        ...
        Given: :test_adapter
        """,
        fn -> EctoLens.get_table(Test.BadRepo, "users") end
      )
    end
  end

  describe "get_table/3 (Postgres)" do
    test "given valid table name and repo, returns EctoLens Table" do
      assert %EctoLens.Table{name: "orgs", schema: "public"} =
               EctoLens.get_table(Test.Postgres.Repo, "orgs")
    end

    test "given invalid table name, but valid repo, returns nil" do
      assert is_nil(EctoLens.get_table(Test.Postgres.Repo, "passports"))
    end

    test "returns nothing when querying table belonging to non-default prefix when not specified" do
      assert is_nil(EctoLens.get_table(Test.Postgres.Repo, "events"))
    end

    test "returns nothing when querying table belonging to incorrectly specified prefix" do
      assert is_nil(EctoLens.get_table(Test.Postgres.Repo, "events", prefix: "something_random"))
    end

    test "returns table belonging to non-default prefix if specified correctly" do
      assert %EctoLens.Table{name: "events", schema: "debug"} =
               EctoLens.get_table(Test.Postgres.Repo, "events", prefix: "debug")
    end

    test "given `async: false` option, derives preloads synchronously and returns table" do
      assert %EctoLens.Table{name: "orgs", schema: "public"} =
               EctoLens.get_table(Test.Postgres.Repo, "orgs", async: false)
    end
  end

  describe "list_tables/2 (Postgres)" do
    test "lists tables and metadata when given valid repo", ctx do
      assert tables = EctoLens.list_tables(Test.Postgres.Repo)

      assert Enum.count(tables) == 5

      assert %EctoLens.Table{} = schema_migrations = ctx.find.(tables, "schema_migrations")
      assert %EctoLens.Table{} = orgs = ctx.find.(tables, "orgs")
      assert %EctoLens.Table{} = _accounts = ctx.find.(tables, "accounts")
      assert %EctoLens.Table{} = accounts_orgs = ctx.find.(tables, "accounts_orgs")
      assert %EctoLens.Table{} = repos = ctx.find.(tables, "repos")

      # By default, the standard ecto `schema_migrations` table is created for us and is public. EctoLens does not
      ...
      assert %EctoLens.Column{name: "version", type: "int8"} =
               ctx.find.(schema_migrations.columns, "version")

      assert %EctoLens.Column{name: "inserted_at", type: "timestamp"} =
               ctx.find.(schema_migrations.columns, "inserted_at")

      assert %EctoLens.Index{name: "schema_migrations_pkey"} =
    ...
    test "lists tables and metadata for all tables with column" do
      ...
      assert [%EctoLens.Table{name: "schema_migrations"}] =
               EctoLens.list_tables(Test.Postgres.Repo, with_column: "version")
    ...
    test "lists tables and metadata for all tables without column", ctx do
      ...
      assert tables = EctoLens.list_tables(Test.Postgres.Repo, without_column: "version")
      assert Enum.count(tables) == 4
      assert is_nil(ctx.find.(tables, "schema_migrations"))

      # Only `schema_migrations` lacks an `updated_at`
      assert tables = EctoLens.list_tables(Test.Postgres.Repo, without_column: "updated_at")
    ...
    test "lists tables and metadata for all tables with foreign key constraint", ctx do
      ...
      assert tables =
               EctoLens.list_tables(Test.Postgres.Repo, with_foreign_key_constraint: "accounts")
    ...
    test "lists tables and metadata for all tables with index", ctx do
      ...
      # indexed by default.
      assert tables_1 = EctoLens.list_tables(Test.Postgres.Repo, with_index: "id")
      assert Enum.count(tables_1) == 3

      assert tables_2 = EctoLens.list_tables(Test.Postgres.Repo, with_column: "id")
    ...
    test "lists tables and metadata for all tables with compound index" do
      ...
      assert [%EctoLens.Table{name: "accounts_orgs"}] =
               EctoLens.list_tables(Test.Postgres.Repo, with_index: ["account_id", "org_id"])

      # Compound index lookup is order sentitive
      assert [] = EctoLens.list_tables(Test.Postgres.Repo, with_index: ["org_id", "account_id"])

      # And lookups of a single component will not work -- the full index must be given at all times
      assert [] = EctoLens.list_tables(Test.Postgres.Repo, with_index: "account_id")
    end
    ...
    test "lists tables and metadata for all tables without index", ctx do
      ...
      assert tables = EctoLens.list_tables(Test.Postgres.Repo, without_index: "id")
      assert Enum.count(tables) == 2
      refute is_nil(ctx.find.(tables, "schema_migrations"))
      refute is_nil(ctx.find.(tables, "accounts_orgs"))

      # No tables defining an `id` field lack an index on `id`
      assert [] = EctoLens.list_tables(Test.Postgres.Repo, with_column: "id", without_index: "id")
    ...
    test "lists tables and metadata for all tables without compound index", ctx do
      ...
      assert tables =
               EctoLens.list_tables(Test.Postgres.Repo, without_index: ["account_id", "org_id"])
    ...
    test "lists tables and metadata with given table name filters", ctx do
      assert [] = EctoLens.list_tables(Test.Postgres.Repo, table_name: "doesn't exist")
    ...
    test "lists tables and metadata with given table name filters (simple regex)" do
      assert [] = EctoLens.list_tables(Test.Postgres.Repo, table_name: ~r/Accounts/)
      assert tables = EctoLens.list_tables(Test.Postgres.Repo, table_name: ~r/Accounts/i)

      for table <- tables, do: assert(table.name =~ "accounts")
    end

    test "lists tables and metadata with given column filters (complex regex)" do
      assert tables = EctoLens.list_tables(Test.Postgres.Repo, with_column: ~r/(^|_)id/)
    ...
    test "lists tables and metadata for all tables having an index covering field", ctx do
      ...
      assert tables_covering_org_id =
               EctoLens.list_tables(Test.Postgres.Repo, with_index_covering: "org_id")
      ...
      assert tables_without_index_covering_account_id =
               EctoLens.list_tables(Test.Postgres.Repo, without_index_covering: "account_id")
    ...
    test "index metadata contains flags `is_unique` and `is_primary`", ctx do
      ...
      assert %EctoLens.Table{} = accounts = ctx.find.(tables, "accounts")
    ...
    test "colum type metadata is returned alongside tables" do
      assert %EctoLens.Table{columns: columns} = EctoLens.get_table(Test.Postgres.Repo, "repos")

      assert %EctoLens.Column{
               type_metadata: %EctoLens.Column.Postgres.Type.Metadata.Character{
      ...
      assert %EctoLens.Column{
               type_metadata: %EctoLens.Column.Postgres.Type.Metadata.Numeric{
                 precision: 64,
                 radix: 2,
                 scale: 0
               }
      ...
      assert %EctoLens.Column{
               type_metadata: %EctoLens.Column.Postgres.Type.Metadata.DateTime{precision: 0}
             } = Enum.find(columns, &(&1.name == "inserted_at"))

      assert %EctoLens.Column{
               type_metadata: %EctoLens.Column.Postgres.Type.Metadata.Interval{
  ...
  describe "load_indexes/1" do
    test "given an empty list, returns empty list" do
      assert [] = EctoLens.load_indexes([])
    end

    test "given a list that contains data other than EctoLens Tables, raises" do
      assert_raise(ArgumentError, fn -> EctoLens.load_indexes([%EctoLens.Table{}, 123]) end)
    end

    test "given a list that contains data other than EctoLens Columns, raises" do
      assert_raise(ArgumentError, fn -> EctoLens.load_indexes([%EctoLens.Column{}, 123]) end)
    end

    test "given a list of tables, fetches all indexes for each column", ctx do
      raw_tables = EctoLens.list_tables(Test.Postgres.Repo)

      for table <- raw_tables, column <- table.columns do
        assert is_struct(column.indexes, EctoLens.Index.NotLoaded)
      end

      tables = EctoLens.load_indexes(raw_tables)
      ...
      assert %EctoLens.Column{
               indexes: [%EctoLens.Index{is_unique: false, name: "accounts_inserted_at_index"}]
             } = Enum.find(accounts.columns, &(&1.name == "inserted_at"))

      assert %EctoLens.Column{
               indexes: [%EctoLens.Index{is_unique: false, name: "accounts_updated_at_index"}]
      ...
      assert %EctoLens.Column{
               indexes: [
                 %EctoLens.Index{is_unique: true, name: "repos_account_id_name_index"} =
    ...
    test "given a single table, returns whether columns are nullable" do
      accounts_orgs = EctoLens.get_table(Test.Postgres.Repo, "accounts_orgs")
    ...
    test "given a single table, fetches all indexes for each column" do
      repos = EctoLens.get_table(Test.Postgres.Repo, "repos")

      for column <- repos.columns do
        assert is_struct(column.indexes, EctoLens.Index.NotLoaded)
      end

      repos = EctoLens.load_indexes(repos)
    ...
    test "given a list of columns, fetches all indexes for each column" do
      ...
      columns = EctoLens.load_indexes(repos.columns)
      ...
      assert %EctoLens.Column{indexes: []} = Enum.find(columns, &(&1.name == "some_interval"))
    end
    ...
    test "given a single column, fetches all indexes for each column" do
      ...
      assert %EctoLens.Column{
               indexes: [%EctoLens.Index{is_unique: true, name: "repos_account_id_name_index"}]
             } = EctoLens.load_indexes(Enum.find(repos.columns, &(&1.name == "account_id")))
    end
    ...
    test "idempotent when called multiple times" do
      ...
      assert %{indexes: [1, 2, 3, 4, 5, 6]} = EctoLens.load_indexes(altered_example)
      assert [%{indexes: [1, 2, 3, 4, 5, 6]}] = EctoLens.load_indexes([altered_example])
    end
  end
  ...
  describe "load_tables/1" do
    test "given a list that contains data other than EctoLens Tables, raises" do
      assert_raise(ArgumentError, fn -> EctoLens.load_schemas([%EctoLens.Table{}, 123]) end)
    end

    test "given single EctoLens Table, loads but sets schemas to empty list if it has no schema impl." do
      assert %EctoLens.Table{schemas: %EctoLens.Schema.NotLoaded{}} =
               accounts = EctoLens.get_table(Test.Postgres.Repo, "accounts")

      assert %EctoLens.Table{schemas: []} = EctoLens.load_schemas(accounts)
    end

    test "given single EctoLens Table, loads schemas where they exist" do
      assert %EctoLens.Table{schemas: %EctoLens.Schema.NotLoaded{}} =
               orgs = EctoLens.get_table(Test.Postgres.Repo, "orgs")

      assert %EctoLens.Table{schemas: [Test.Postgres.Org]} = EctoLens.load_schemas(orgs)
    end

    test "loads the corresponding Ecto Schemas of all tables in list" do
      assert tables = EctoLens.list_tables(Test.Postgres.Repo)
      assert Enum.all?(tables, &is_struct(&1.schemas, EctoLens.Schema.NotLoaded))

      assert tables = EctoLens.load_schemas(tables)
      refute Enum.all?(tables, &is_struct(&1.schemas, EctoLens.Schema.NotLoaded))
